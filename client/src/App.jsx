...
function LanPong() {
  ...
  // Inputs
  const keysRef = useRef({ up: false, down: false, action: false, powerKey: null });
  const inputP1Ref = useRef({ up: false, down: false, action: false, powerKey: null });
  const inputP2Ref = useRef({ up: false, down: false, action: false, powerKey: null });
  ...
  const powerupOrder = ["dash", "gravityWell", "decoyBall", "parry"];
  const powerKeyMap = { 1: ["1", "2", "3", "4"], 2: ["7", "8", "9", "0"] };
  ...
  const createPowerups = () => ({
    ...
    decoyBall: {
      key: "3",
      name: "Decoy Ball",
      desc: "Spawn a fake ball that fools opponents",
      unlocked: false,
      charges: 0,
      cooldownUntil: 0,
    },
  ...
  const challengeIds = Object.keys(challengeDefs);
  ...
  const connect = () => {
    ...
    ws.onmessage = (ev) => {
      ...
      if (msg.type === "input") {
        ...
        if (!isHost) return;
        const from = msg.from;
        const target = from === 1 ? inputP1Ref.current : inputP2Ref.current;
        if (!target) return;
        if ("up" in msg) target.up = !!msg.up;
        if ("down" in msg) target.down = !!msg.down;
        if (msg.action) target.action = true;
        if (msg.powerKey) target.powerKey = msg.powerKey;
        return;
      }
    ...
    const onKeyDown = (e) => {
      ...
      if (player === 1) {
        ...
        if (k === "e" || k === "E") {
          ...
          wsRef.current?.send(JSON.stringify({ type: "input", action: true }));
        }
        if (powerKeyMap[1].includes(k)) {
          keysRef.current.powerKey = k;
          wsRef.current?.send(JSON.stringify({ type: "input", powerKey: k }));
        }
      }
      if (player === 2) {
  ...
  useEffect(() => {
    ...
    const pickNextChallenge = (g, side) => {
      const prevId = g.players[side].challengeId;
      const options = challengeIds.filter((id) => id !== prevId);
      const next = options[Math.floor(Math.random() * options.length)];
      g.players[side].challengeId = next;
      g.players[side].challengeDone = false;
      g.players[side].challengeStartedAt = performance.now();
      challengeDefs[next]?.init?.(g, side);
    };

    const awardChallengeReward = (g, side) => {
      const powerups = g.players[side].powerups;
      const locked = powerupOrder.find((k) => !powerups[k].unlocked);
      if (locked) {
      ...
      const candidates = powerupOrder.filter((k) => powerups[k].charges < cfg.powerupChargeMax);
    ...
    const tryActivatePowerup = (g, side, key, input) => {
      const now = performance.now();
      const powerups = g.players[side].powerups;
      const resolvePowerupId = () => {
        if (side === "L") {
          return powerupOrder.find((id) => powerups[id].key === key);
        }
        const idx = powerKeyMap[2].indexOf(key);
        return idx >= 0 ? powerupOrder[idx] : null;
      };
      const id = resolvePowerupId();
      if (!id) return;
      const p = powerups[id];
      ...
      if (id === "gravityWell") {
        ...
        spawnPop(g, cfg.w / 2, g.players[side].gravity.y, 18);
      }

      if (id === "decoyBall") {
        g.players[side].ghostBall = {
          x: g.ball.x,
          y: g.ball.y,
          vx: g.ball.vx,
    ...
    const handleAction = (g, side) => {
      const id = g.players[side].challengeId;
      if (!id) return;
      const ok = challengeDefs[id]?.onAction?.(g, side);
      if (ok) {
        ...
        const paddle = side === "L" ? g.left : g.right;
        spawnPop(g, paddle.x + (side === "L" ? cfg.paddleW + 20 : -20), paddle.y + cfg.paddleH / 2, 6);
      }
    };
    ...
    const updateHost = (g, dt) => {
      ...
      if (player === 1) {
        i1.up = local.up;
        i1.down = local.down;
        if (local.action) i1.action = true;
        if (local.powerKey) i1.powerKey = local.powerKey;
      }
      if (player === 2) {
        i2.up = local.up;
        i2.down = local.down;
        if (local.action) i2.action = true;
        if (local.powerKey) i2.powerKey = local.powerKey;
      }
      ...
      // init challenges if missing
      if (!g.players.L.challengeId) pickNextChallenge(g, "L");
      if (!g.players.R.challengeId) pickNextChallenge(g, "R");

      // challenge tick
      challengeDefs[g.players.L.challengeId]?.tick?.(g, "L", dt, i1);
      challengeDefs[g.players.R.challengeId]?.tick?.(g, "R", dt, i2);

      // action/power inputs
      if (i1.action) {
        handleAction(g, "L");
        i1.action = false;
        if (player === 1) keysRef.current.action = false;
      }
      if (i2.action) {
        handleAction(g, "R");
        i2.action = false;
        if (player === 2) keysRef.current.action = false;
      }
      if (i1.powerKey) {
        tryActivatePowerup(g, "L", i1.powerKey, i1);
        i1.powerKey = null;
        if (player === 1) keysRef.current.powerKey = null;
      }
      if (i2.powerKey) {
        tryActivatePowerup(g, "R", i2.powerKey, i2);
        i2.powerKey = null;
        if (player === 2) keysRef.current.powerKey = null;
      }
      ...
      for (const side of ["L", "R"]) {
        const ghost = g.players[side].ghostBall;
      ...
      const bounced = bounceWall();
      if (bounced && g.lastHitBy) {
        g.players[g.lastHitBy].lastWallBounceAt = performance.now();
      }

      // Gate crossing check (midline)
      const crossedMid =
        (g.ball.vx > 0 && g.ball.x >= cfg.w / 2 && g.ball.x - g.ball.vx * dt < cfg.w / 2) ||
        (g.ball.vx < 0 && g.ball.x <= cfg.w / 2 && g.ball.x - g.ball.vx * dt > cfg.w / 2);
      if (crossedMid) {
        if (g.players.L.challengeId === "movingGate" && g.ball.vx > 0) {
          if (challengeDefs.movingGate.onBallCrossMid(g, "L")) g.players.L.challengeDone = true;
        }
        if (g.players.R.challengeId === "movingGate" && g.ball.vx < 0) {
          if (challengeDefs.movingGate.onBallCrossMid(g, "R")) g.players.R.challengeDone = true;
        }
      }

      // Gravity well effects
      for (const side of ["L", "R"]) {
        const gravity = g.players[side].gravity;
        if (gravity && performance.now() < gravity.until) {
          const dx = gravity.x - g.ball.x;
          const dy = gravity.y - g.ball.y;
          const dist = Math.max(60, Math.hypot(dx, dy));
          const pull = (gravity.strength / dist) * dt;
      ...
      const hit = (p, side) => {
        ...
        // parry boost
        if (performance.now() < g.players[side].parryWindowUntil) {
          g.ball.vx *= 1.12;
          g.ball.vy *= 1.06;
          g.ball.vy += off * 220;
      ...
      const hitLeft = hit(g.left, "L");
      const hitRight = hit(g.right, "R");

      if (hitLeft && g.players.R.challengeId === "wallTrick") {
        if (challengeDefs.wallTrick.onOpponentHit(g, "R")) g.players.R.challengeDone = true;
      }
      if (hitRight && g.players.L.challengeId === "wallTrick") {
        if (challengeDefs.wallTrick.onOpponentHit(g, "L")) g.players.L.challengeDone = true;
      }
      ...
      const onPoint = () => {
        for (const side of ["L", "R"]) {
          g.players[side].actionPulseAt = 0;
          g.players[side].gate = null;
          g.players[side].stillTime = 0;
          g.players[side].lastWallBounceAt = 0;
          g.players[side].parryWindowUntil = 0;
          g.players[side].gravity = null;
          g.players[side].ghostBall = null;
        }
        g.lastHitBy = null;
        setTimeout(() => {
          const gg = gRef.current;
          if (!gg || gg.winner) return;
      ...
      if (!g.players.L.challengeDone && challengeDefs[g.players.L.challengeId]?.check?.(g, "L")) {
        g.players.L.challengeDone = true;
      }
      if (!g.players.R.challengeDone && challengeDefs[g.players.R.challengeId]?.check?.(g, "R")) {
        g.players.R.challengeDone = true;
      }

      if (g.players.L.challengeDone) {
        g.players.L.challengeDone = false;
        spawnPop(g, cfg.w / 2, 120, 22);
        awardChallengeReward(g, "L");
        setTimeout(() => {
          const gg = gRef.current;
          if (gg && !gg.winner) pickNextChallenge(gg, "L");
        }, 650);
      }
      if (g.players.R.challengeDone) {
        g.players.R.challengeDone = false;
        spawnPop(g, cfg.w / 2, 120, 22);
        awardChallengeReward(g, "R");
        setTimeout(() => {
          const gg = gRef.current;
          if (gg && !gg.winner) pickNextChallenge(gg, "R");
        }, 650);
      }
    };
    ...
    const drawPowerupBar = (g, side) => {
      const now = performance.now();
      const powerups = g.players[side].powerups;
      const order = powerupOrder;
      const w = 200;
      const h = 28;
      let x = side === "L" ? 18 : cfg.w - 18 - w;
      const y = side === "L" ? 18 : 52;
      ...
      for (const id of order) {
        ...
        ctx.textAlign = "left";
        const keyLabel = side === "L" ? p.key : powerKeyMap[2][order.indexOf(id)];
        ctx.fillText(`${keyLabel} ${p.name}`, x + 10, y + 18);
        ...
        if (locked) {
          ...
          ctx.fillText(`${secs.toFixed(1)}s`, x + w - 10, y + 18);
        } else {
          ctx.fillStyle = charges > 0 ? "rgba(231,236,255,0.9)" : "rgba(231,236,255,0.55)";
          ctx.fillText(`x${charges}`, x + w - 10, y + 18);
        }
    ...
    const draw = (g, overlayText = "") => {
      ...
      // moving gates
      for (const side of ["L", "R"]) {
        if (g.players[side].gate && g.players[side].challengeId === "movingGate") {
          const gate = g.players[side].gate;
          ctx.strokeStyle = side === "L" ? "rgba(231,236,255,0.35)" : "rgba(176,214,255,0.35)";
          ctx.lineWidth = 3;
          ctx.beginPath();
          ctx.moveTo(gate.x, gate.y);
          ctx.lineTo(gate.x, gate.y + gate.h);
          ctx.stroke();
          ctx.fillStyle = side === "L" ? "rgba(231,236,255,0.12)" : "rgba(176,214,255,0.12)";
      ...
      for (const side of ["L", "R"]) {
        ...
        if (gravity && performance.now() < gravity.until) {
          ...
          ctx.lineWidth = 2;
          ctx.beginPath();
          ctx.arc(gravity.x, gravity.y, 42, 0, Math.PI * 2);
          ctx.stroke();
          ctx.strokeStyle = side === "L" ? "rgba(231,236,255,0.18)" : "rgba(176,214,255,0.18)";
          ctx.beginPath();
          ctx.arc(gravity.x, gravity.y, 72, 0, Math.PI * 2);
          ctx.stroke();
        }
      }
      ...
      // parry indicators
      if (performance.now() < g.players.L.parryWindowUntil) {
      ...
      if (performance.now() < g.players.R.parryWindowUntil) {
        ctx.strokeStyle = "rgba(176,214,255,0.45)";
        ctx.lineWidth = 2;
        ctx.beginPath();
      ...
      for (const side of ["L", "R"]) {
        const ghost = g.players[side].ghostBall;
        if (ghost && performance.now() < ghost.until) {
          ctx.fillStyle = side === "L" ? "rgba(231,236,255,0.22)" : "rgba(176,214,255,0.22)";
          ctx.beginPath();
      ...
      const leftChallenge = challengeDefs[g.players.L.challengeId];
      const rightChallenge = challengeDefs[g.players.R.challengeId];
      if (leftChallenge) {
        ctx.fillStyle = "rgba(231,236,255,0.75)";
        ctx.font = "600 12px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto";
        ctx.textAlign = "left";
        ctx.fillText(`P1: ${leftChallenge.title}`, 18, cfg.h - 34);
      }
      if (rightChallenge) {
        ctx.fillStyle = "rgba(176,214,255,0.75)";
        ctx.font = "600 12px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto";
        ctx.textAlign = "right";
        ctx.fillText(`P2: ${rightChallenge.title}`, cfg.w - 18, cfg.h - 34);
      }

      if (g.players.L.challengeId === "actionTiming" && g.players.L.actionPulseAt > 0) {
        ctx.fillStyle = "rgba(231,236,255,0.9)";
        ctx.font = "900 18px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto";
        ctx.textAlign = "center";
        ctx.fillText("P1 PRESS E", cfg.w * 0.22, cfg.h * 0.18);
      }
      if (g.players.R.challengeId === "actionTiming" && g.players.R.actionPulseAt > 0) {
        ctx.fillStyle = "rgba(176,214,255,0.9)";
        ctx.font = "900 18px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto";
        ctx.textAlign = "center";
        ctx.fillText("P2 PRESS ENTER", cfg.w * 0.78, cfg.h * 0.18);
      }
    ...
    const step = (now) => {
      ...
      if (isHost) {
        ...
        draw(g, `Room: ${room} • You: P${player} ${isHost ? "(host)" : ""} • W/S + E/1-4 vs ↑/↓ + Enter/7-0`);
        return;
      }
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 10, opacity: 0.75, fontSize: 13 }}>
        Controls: P1 W/S + E + 1-4 • P2 ↑/↓ + Enter + 7-0 • Space toggles local pause (host pauses simulation)
      </div>
    </div>
  );
}
